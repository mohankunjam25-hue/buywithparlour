...
class ProductRegistry {
    ...
    static async addProduct(data) {
        const newId = (data._id && mongoose_1.default.Types.ObjectId.isValid(data._id))
            ? String(data._id)
            : new mongoose_1.default.Types.ObjectId().toHexString();
        const isDraft = data.approvalStatus === 'DRAFT';
        const categoryId = (data.category && typeof data.category === 'object' && data.category._id && mongoose_1.default.Types.ObjectId.isValid(data.category._id))
            ? data.category._id
            : (data.category && typeof data.category === 'string' && mongoose_1.default.Types.ObjectId.isValid(data.category))
                ? data.category
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await product_model_1.ProductModel.create({
                    _id: new mongoose_1.default.Types.ObjectId(newId),
                    title: product.title,
                    slug: product.slug,
                    brand: product.brand,
                    price: product.price,
    ...
    static async updateProduct(id, updates) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await product_model_1.ProductModel.findByIdAndUpdate(id, item, { new: true });
            }
            catch (err) {
     ...
     */
    static async getSellerProducts(status) {
    ...
    static async approveProduct(id) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await product_model_1.ProductModel.findByIdAndUpdate(id, { approvalStatus: 'APPROVED', isPublished: true, rejectionReason: undefined }, { new: true });
            }
            catch (err) {
    ...
    static async rejectProduct(id, reason) {
        ...
        if (mongoose_1.default.connection.readyState === 1) {
            try {
                await product_model_1.ProductModel.findByIdAndUpdate(id, { approvalStatus: 'REJECTED', isPublished: false, rejectionReason: reason || 'Does not satisfy marketplace quality standards.' }, { new: true });
            }
            catch (err) {
