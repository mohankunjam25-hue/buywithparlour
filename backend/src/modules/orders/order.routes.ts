...
import { createPaymentGatewayOrder, verifyPaymentSignature } from '../../services/payment.service';
import { sendResponse, sendError } from '../../utils/response';

const router = Router();

// PUBLIC PAYMENT ENDPOINTS (Allows guest & logged-in checkout)
router.post('/payment/create-order', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { amount, receipt } = req.body;
    if (!amount) {
      sendError(res, 400, 'Amount in INR is required.');
      return;
    }

    const orderData = await createPaymentGatewayOrder(amount, receipt || `receipt_${Date.now()}`);
    sendResponse(res, 200, true, 'Payment Order Created', { paymentOrder: orderData });
  } catch (error) {
    next(error);
  }
});

router.post('/payment/verify', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!isValid) {
      sendError(res, 400, 'Invalid payment signature. Payment verification failed.');
      return;
    }
...
router.post('/track', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!orderNumber || typeof orderNumber !== 'string') {
      sendError(res, 400, 'Order Number (e.g. BP-...) is required.');
      return;
    }

    const cleanOrderNumber = orderNumber.trim();
    let order = await OrderModel.findOne({ orderNumber: cleanOrderNumber });

    if (!order && cleanOrderNumber.startsWith('BP-')) {
      // Check case-insensitively
      order = await OrderModel.findOne({ orderNumber: new RegExp(`^${cleanOrderNumber}$`, 'i') });
    }

    if (!order) {
      sendError(res, 404, `No active parcel found for Order ID: ${cleanOrderNumber}. Please check the Order ID.`);
      return;
    }
    ...
    sendResponse(res, 200, true, 'Live shipment tracking fetched', { tracking: trackingPayload });
  } catch (error) {
    next(error);
  }
});
...
router.patch('/:id/status', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!validStatuses.includes(status)) {
      sendError(res, 400, `Invalid order status. Must be one of: ${validStatuses.join(', ')}`);
      return;
    }
    ...
    sendResponse(res, 200, true, `Order status updated to ${status}`, { order });
  } catch (error) {
    next(error);
  }
});
