...
export class ProductService {
  ...
  static async getProducts(query: {
    ...
    if (query.category) {
      if (mongoose.Types.ObjectId.isValid(query.category)) {
        ...
        try {
          const cat = await CategoryModel.findOne({ slug: query.category.toLowerCase().trim() });
          if (cat) {
    ...
    if (query.sort === 'rating') sortOption = { rating: -1, totalReviews: -1 };
    if (query.sort === 'popular') sortOption = { totalReviews: -1, rating: -1 };
    if (query.sort === 'discount') sortOption = { discountPrice: 1 };
    ...
    const liveItems = await ProductRegistry.getLiveProducts();
    ...
    if (query.brand) {
      filtered = filtered.filter((p) =>
        query.brand!.toLowerCase().split(',').includes(p.brand.toLowerCase())
      );
    }
    ...
    return {
      products: filtered as unknown as IProduct[],
   ...
   */
  static async getSuggestions(keyword: string) {
    ...
    if (mongoose.connection.readyState === 1) {
      const [matchingProducts, matchingCategories] = await Promise.all([
        ProductModel.find({
          ...
          .limit(6)
          .lean(),
        CategoryModel.find({ name: regex }).select('name slug').limit(3).lean(),
      ]);
  ...
  static async createProduct(data: Partial<IProduct>): Promise<IProduct> {
    return (await ProductRegistry.addProduct(data as any)) as unknown as IProduct;
  }
  ...
  static async updateProduct(id: string, data: Partial<IProduct>): Promise<IProduct> {
    if (mongoose.connection.readyState === 1) {
      ...
      if (updated) return updated as unknown as IProduct;
    }
    const liveItems = await ProductRegistry.getLiveProducts();
    const prod = liveItems.find((p) => p._id === id);
    if (!prod) throw new Error('Product not found');
    Object.assign(prod, data);
    return prod as unknown as IProduct;
  }
